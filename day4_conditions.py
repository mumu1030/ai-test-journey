...
#使用普通方法，练习筛选并打印P0优先级的用例
print("\n"+"="*20)
print("使用普通方法筛选P0优先级用例和统计条数")
print("="*20)
test_cases_P0 = [] #建个空列表装P0优先级的用例
for case in test_cases: #把用例逐条遍历
    if case["priority"] == "P0": #判断条件
        test_cases_P0.append(case) #把用例加到空列表
        print(f"{case['id']},{case['title']},{case['priority']}:{case['status']}")#并且把刚加入的用例的值
print(f"PO用例:{len(test_cases_P0)}条") #统计这个p0空列表的长度

print("="*20)
print("统计测试通过率") #只算pass和fail的用例，skip不算

pass_count = 0 #定义通过用例变量初始值0，计算数量
fail_count = 0 #定义失败用例变量初始值0，计算数量
...
print(f"测试通过率为：{pass_rate:.1f}%")

#使用列表推导式，练习筛选并打印P0优先级的用例
print("="*20)
print("使用列表推导式筛选并打印P0优先级的用例")

test_cases_P0 = [case for case in test_cases if case["priority"] == "P0"] #遍历原始数据带判断构建p0用例列表

for case in test_cases_P0: #把每条用例逐条遍历
    print(f"P0用例:{case['id']},{case['title']},{case['title']}:{case['status']}") #按要求取值打印
...
test_cases_pass = [case for case in test_cases if case["status"] == "pass"]#遍历原始数据带判断构建pass用例列表
print("\n")
print("="*20)
print("通过的用例")
for case in test_cases_pass:#把每条用例逐条遍历
